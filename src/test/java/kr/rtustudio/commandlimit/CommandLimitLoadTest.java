...
class CommandLimitLoadTest extends BaseRSPluginTest<CommandLimit> {

    @Override
    protected CommandLimit createPluginMock() {
        return loadPlugin(CommandLimit.class);
    }
    ...
    @Test
    @DisplayName("명령어(/cmdlimit)가 정상적으로 등록되어 있다")
    void should_register_command() {
        verifyCommand("cmdlimit");
    }
    ...
    void should_execute_command_without_exception() {
        ...
        try {
            player.performCommand("cmdlimit");
